...
class HomeController extends Controller
{
    ...
    public function category($slug, $category_id, Request $request)
    {
        ...
        if ($name == 'popullar') {
            $products = Product::where('category_id', $category_id)->Where('popular_status', 1)->latest()->paginate(8)->appends(request()->query());
    ...
    public function brand($slug, $brand_id, Request $request)
    {
        ...
        if ($name == 'popullar') {
            $products = Product::where('brand_id', $brand_id)->Where('popular_status', 1)->latest()->paginate(8)->appends(request()->query());
        } elseif ($name == 'cheapest') {
    ...
    public function search(Request $request)
    {
        ...
        if(isset($_GET['start-price']) && $_GET['end-price']){
            ...
            $products = Product::search()->paginate(8)->appends(request()->query());
        }
    ...
    public function productdetails($id)
    {
        ...
        $rating_count = Comment::where('products_id', $id)->get();
        $rating_count1 = Comment::where('products_id', $id)->get()->count();
        $rating_sum = Comment::where('products_id', $id)->sum('rating');
        if($rating_count1 > 0){
